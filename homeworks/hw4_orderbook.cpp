...
struct Order{ // public
    ...
    Order(
            ...
            type(type_)
    {
        std::strcpy(venue, venue_);
    }
    Order(const Order &oo)
    {
...
public:
    ...
    void add_order(Order& o) // Order reference called "o"
    {
        ...
        if (o.type==IOC) {display_error_IOC(o); return;}
        ...
        std::unordered_map<order_id_t, Order *>& book{ o.is_bid ? bids : asks};
        ...
        if ( book.find(o.id)!=book.end() ) {display_error_order_id_exist(o); return;}

        Order* add_o {new Order(o)};
    ...
    void modify_order(struct Order o){
        ...
        std::unordered_map<order_id_t, Order *>::iterator bookit { book.find(o.id) };
        if ( bookit==book.end() ) {display_error_exist_order_modification(o); return;}



        // if the modification coming from the order in the parameter of the function has a different price
        // you shouldn't process the amendment and you should display an error
        if (bookit->second->price != o.price) {display_error_amendment_price(o); return;}


        // if the quantity of the order in the parameter of this function is not lower than the original quantity
        // you shouldn't process the order and you should display an error
        if (bookit->second->quantity <= o.quantity) {
            display_error_amendment_quantity(o); return;}
        ...
        bookit->second->quantity = o.quantity;
    ...
    void delete_order(struct Order o)
    {
        ...
        if ( bookit==book.end() ) {display_error_order_not_existing_deletion(o); return;}
    ...
    void process_order(OrderAction o)
    {
        ...
        switch (o.action)
        {
            case CREATE : add_order(o.order); break;
            case DELETE : delete_order(o.order); break;
            case MODIFY : modify_order(o.order); break;
        }
    ...
    void get_best_bid(){
        ...
        for (auto const& elem: bids)
        {
            best = (elem.second->price > best) ? elem.second->price : best;
        }
        if (best) std::cout << "best bid:" << best << '\n';
        else std::cout << "no bids\n";
    };
    ...
    void get_best_ask(){
        ...
        price_t best{1000000};

        for (auto const& elem: asks)
        {
            best = (elem.second->price < best) ? elem.second->price : best;
        }
        if (best!=1000000) std::cout << "best ask:" << best << '\n';
        else std::cout << "no asks\n";
    };
    ...
    void clear_book(){
        ...
        // all the orders
        auto number_of_bids_deleted{ bids.size() }, number_of_asks_deleted{ asks.size() };
        bids.clear();
        asks.clear();
